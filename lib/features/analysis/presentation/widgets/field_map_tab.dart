...
class _FieldMapTabState extends State<FieldMapTab> {
  ...
  Widget build(BuildContext context) {
    ...
    if (players.isEmpty) {
      return Center(
        child: Text(
          l10n.fieldNoPlayerData,
          style: TextStyle(color: c.muted),
        ),
      );
    }

    final maxKm = players.fold<double>(
      0,
      (p, e) => math.max(p, (e['distance_km'] as num?)?.toDouble() ?? 0),
    );
    final teamPoss = players.fold<double>(
      0,
      (s, p) => s + ((p['possession_pct'] as num?)?.toDouble() ?? 0),
    );
    final oppPoss = math.max(0.0, 100.0 - teamPoss);
    final formation = _detectFormation(players);
    final selected = _selectedRank != null
        ? players.firstWhere(
            (p) => p['rank'] == _selectedRank,
  ...
  String _detectFormation(List<Map<String, dynamic>> players) {
    ...
    final sorted = [...players]
      ..sort(
        (a, b) =>
            (b['avg_y_norm'] as num).compareTo(a['avg_y_norm'] as num),
      );

    final outfield = sorted.skip(1).toList();

    final defs =
        outfield.where((p) => (p['avg_y_norm'] as num) > 0.62).length;
    ...
    final atts =
        outfield.where((p) => (p['avg_y_norm'] as num) < 0.38).length;
  ...
  String _avgSpeed(List<Map<String, dynamic>> players) {
    ...
    final total = players.fold<double>(
      ...
      (s, p) =>
          s +
          ((p['speed_kmh'] as num?)?.toDouble() ??
              (p['speed_ms'] as num?)?.toDouble() ??
              0),
    );
...
class _TacticalPitchPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < stripes; i++) {
      stripePaint.color =
          i.isEven ? const Color(0xFF0D1F10) : const Color(0xFF0B1A0D);
      canvas.drawRect(
        Rect.fromLTWH(i * w / stripes, 0, w / stripes, h),
        stripePaint,
      );
    }
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(fieldRect, const Radius.circular(3)),
      line,
    );
    ...
    canvas.drawRect(
      Rect.fromLTWH(m - 5, goalT, 5, goalH),
      line..strokeWidth = 1.5,
    );
    canvas.drawRect(
      Rect.fromLTWH(w - m, goalT, 5, goalH),
      line..strokeWidth = 1.5,
    );
    ...
    canvas.drawCircle(
      Offset(m + pbW * 0.75, h / 2),
      2.5,
      Paint()..color = Colors.white.withValues(alpha: 0.35),
    );
    canvas.drawCircle(
      Offset(w - m - pbW * 0.75, h / 2),
      2.5,
      Paint()..color = Colors.white.withValues(alpha: 0.35),
    );
    ...
    for (final p in players) {
      ...
      final textColor =
          ratio > 0.45 ? const Color(0xFF0B1A0D) : Colors.white.withValues(alpha: 0.9);
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    l10n.fieldPlayerLabel(rank, zone),
                    style: TextStyle(
                      color: c.textHi,
                      fontSize: 14,
                      fontWeight: FontWeight.w700,
